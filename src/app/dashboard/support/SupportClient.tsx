...
const faqs = [
  { 
    id: 1, 
    question: "Bagaimana cara melakukan pesanan khusus (Custom B2B)?", 
    answer: "Untuk pesanan custom B2B, Anda dapat menghubungi tim sales kami langsung melalui email atau WhatsApp. Anda juga bisa mengunggah desain blueprint (CAD) pada form konsultasi, lalu tim kami akan memberikan estimasi biaya (Quotation) dalam waktu 2x24 jam kerja." 
  },
  { 
    id: 2, 
    question: "Apa syarat pengajuan komplain barang rusak?", 
    answer: "Anda dapat mengajukan komplain maksimal 3x24 jam setelah barang diterima dengan menyertakan video unboxing dan foto detail kerusakan produk melalui menu Komplain." 
  },
  { 
    id: 3, 
    question: "Berapa lama proses produksi dilakukan?", 
    answer: "Waktu produksi standar kami adalah 14-30 hari kerja, tergantung pada kompleksitas desain, jenis material, dan jumlah pesanan yang Anda minta." 
  },
  { 
    id: 4, 
    question: "Bagaimana cara merubah alamat pengiriman?", 
    answer: "Perubahan alamat pengiriman hanya dapat dilakukan sebelum status pesanan berubah menjadi 'Diproses'. Silakan hubungi layanan pelanggan kami secepatnya jika ingin mengubah alamat." 
  },
  { 
    id: 5, 
    question: "Apakah saya bisa mencicil pembayaran Invoice?", 
    answer: "Ya, kami menyediakan opsi pembayaran tempo (Term of Payment) khusus untuk klien B2B yang telah memenuhi syarat dan ketentuan perusahaan kami." 
  }
];
          ...
          <div className="bg-white border border-zinc-200 rounded-xl shadow-sm overflow-hidden">
            {filteredFaqs.length === 0 ? (
              <div className="p-8 text-center text-zinc-500 text-sm">
                Tidak ada topik bantuan yang sesuai dengan pencarian Anda.
              </div>
            ) : (
                ...
                return (
                  <div key={faq.id} className="border-b border-zinc-100 last:border-0">
                    <button 
                      ...
                      className={`w-full px-6 py-5 flex justify-between items-center transition-colors text-left ${isExpanded ? 'bg-zinc-50' : 'hover:bg-zinc-50'}`}
                    >
                      <span className={`text-[14px] ${isExpanded ? 'font-bold text-[#cc4224]' : 'font-semibold text-zinc-900'}`}>
                        {faq.question}
                      </span>
                      <ChevronDown className={`w-5 h-5 transition-transform ${isExpanded ? 'text-[#cc4224] rotate-180' : 'text-zinc-400'}`} />
                    </button>
                    {isExpanded && (
